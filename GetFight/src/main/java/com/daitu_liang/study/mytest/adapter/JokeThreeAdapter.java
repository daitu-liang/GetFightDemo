...
public class JokeThreeAdapter extends CommonAdapter<TypeListEntity.DataBean> {
    ...
    protected void convert(ViewHolder holder, TypeListEntity.DataBean dataBean, int position) {
        ...
        if(dataBean!=null&&dataBean.getGroup()!=null){
            holder.setText(R.id.joke_title_tv, position + "-" + dataBean.getGroup().getContent());


            if(dataBean.getGroup().getMiddle_image()!=null&&dataBean.getGroup().get_$480p_video()!=null){
                TypeListEntity.DataBean.GroupBean._$480pVideoBean vedioInfo = dataBean.getGroup().get_$480p_video();
                String url = vedioInfo.getUrl_list().get(0).getUrl();
                ...
                jcVideoPlayerStandard.setUp(url, JCVideoPlayerStandard.SCREEN_LAYOUT_NORMAL, "");

                String imgeUrlVedio = dataBean.getGroup().getCover_image_url();
